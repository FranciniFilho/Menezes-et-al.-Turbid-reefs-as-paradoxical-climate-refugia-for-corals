...
try:
    import sys; print(f"DEBUG: CWD before load = {os.getcwd()}", file=sys.stderr)
    import sys; print(f"DEBUG: SITES_CSV_PATH = {repr(SITES_CSV_PATH)}", file=sys.stderr)
    import sys; print(f"DEBUG: Path exists = {os.path.exists(SITES_CSV_PATH)}", file=sys.stderr)
    sites_df = pd.read_csv(SITES_CSV_PATH, sep=';')
    print(f"DEBUG: About to call pd.read_csv with {repr(SITES_CSV_PATH)}", file=sys.stderr)
