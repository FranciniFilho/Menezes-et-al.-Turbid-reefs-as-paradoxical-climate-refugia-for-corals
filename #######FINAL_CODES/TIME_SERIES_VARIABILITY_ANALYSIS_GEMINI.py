...
def create_availability_figure(sst_ts_dict, dli_ts_dict, chl_ts_dict, sites_df, output_path):
    ...
    for i, (var_name, ts_dict, color) in enumerate(vars_to_plot):
        ...
        for j, arc in enumerate(['inner', 'outer']):
            ...
            print(f"    {var_name} - {arc}: {len(arc_dates)} unique dates")
            
            for k, date in enumerate(date_range):
                if date.date() in arc_dates:
                    grid[j, k] = 1
        
        # Use bar-like representation instead of imshow for clarity
        for j, arc in enumerate(['inner', 'outer']):
            arc_color = COLOR_INNER if arc == 'inner' else COLOR_OUTER
            dates_with_data = date_range[grid[j, :] == 1]
            ax.scatter(dates_with_data, [j] * len(dates_with_data), s=1, c=arc_color, marker='|')
