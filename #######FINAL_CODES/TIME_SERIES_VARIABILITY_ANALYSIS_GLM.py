### TIME_SERIES_VARIABILITY_ANALYSIS_GLM.py ###
...
# 4. Data Availability Diagnostics (NEW)
#
...
def create_availability_figure(sst_ts_dict, dli_ts_dict, chl_ts_dict, sites_df, output_path):
    """
    Cria figura diagnóstica de disponibilidade temporal (Heatmap).
    """
    print("  Creating Data Availability Figure...")

    # Create broad date range
    start_date = pd.Timestamp(year=START_YEAR, month=1, day=1)
    end_date = pd.Timestamp(year=END_YEAR, month=12, day=31)
    date_range = pd.date_range(start_date, end_date)

    fig, axes = plt.subplots(3, 1, figsize=(15, 8), sharex=True)
    fig.suptitle('Data Availability: Temporal Coverage by Variable and Arc', fontsize=14, fontweight='bold')

    vars_to_plot = [
        ('SST', sst_ts_dict, COLOR_SST),
        ('DLI', dli_ts_dict, COLOR_DLI),
        ('Chl-a', chl_ts_dict, COLOR_CHL)
    ]

    import matplotlib.dates as mdates

    for i, (var_name, ts_dict, color) in enumerate(vars_to_plot):
        ax = axes[i]

        # Binary grid [Inner/Outer x Dates]
        grid = np.zeros((2, len(date_range)))

        for j, arc in enumerate(['inner', 'outer']):
            arc_sites = sites_df[sites_df['Arc'] == arc]['Site_name'].tolist()
            arc_dates = set()
            for site in arc_sites:
                if site in ts_dict:
                    arc_dates.update(ts_dict[site].dropna().index.date)

            for k, date in enumerate(date_range):
                if date.date() in arc_dates:
                    grid[j, k] = 1

        # Plot heatmap
        im = ax.imshow(grid, aspect='auto', interpolation='nearest', cmap='Greys',
                      extent=[mdates.date2num(start_date), mdates.date2num(end_date), -0.5, 1.5])

        ax.set_yticks([0, 1])
        ax.set_yticklabels(['Inner', 'Outer'])
        ax.set_ylabel(var_name, fontsize=12, fontweight='bold', color=color)
...
if __name__ == "__main__":
    ...
    create_availability_figure(sst_ts, dli_ts, chl_ts, sites_df, os.path.join(OUTPUT_DIR, 'Fig_S1_Data_Availability.png'))

    # Print summary to console
    print("\n  Data Coverage Summary:")
    for _, row in diagnostic_df.iterrows():
        print(f"    {row['Variable']} ({row['Arc']}): {row['Coverage_Pct']:.1f}% coverage, Max Gap: {row['Max_Gap_Days']} days")
    ...
    print(f"\n  OK CV calculated for SST: {len(cv_sst)} sites")
    print(f"  OK CV calculated for DLI: {len(cv_dli)} sites")
    print(f"  OK CV calculated for Chl-a: {len(cv_chl)} sites")
    ...
    print(f"\n  OK Spectral analysis for SST: {len(spectral_sst)} sites")
    print(f"  OK Spectral analysis for DLI: {len(spectral_dli)} sites")
    print(f"  OK Spectral analysis for Chl-a: {len(spectral_chl)} sites")
    ...
    print(f"\n  OK Pulse detection for SST: {len(pulse_sst)} sites")
    print(f"  OK Pulse detection for DLI: {len(pulse_dli)} sites")
    print(f"  OK Pulse detection for Chl-a: {len(pulse_chl)} sites")
    ...
    print("  OK Saved: CV_Summary_by_Site.csv")
    ...
    print("  OK Saved: Pulse_Summary_by_Site.csv")
