### ==================================================================================== ###
### === MASTER VISUALIZATION PIPELINE V6 - YEAR RANDOM EFFECT MODELS (INTEGRATED)       === ###
### ===                                                                              === ###
### === BASED ON: 06_MASTER_Viz_Pipeline_YEAR_RE_v5_FINAL.R                          === ###
### ===                                                                              === ###
### === V6 IMPROVEMENTS:                                                              === ###
...
search_directories_YEAR_RE_v6 <- c(
...
output_dir_YEAR_RE_v6 <- "C:/Users/rbfra/OneDrive/Bayesian_Figures_YEAR_RE_v6_INTEGRATED"
dir.create(output_dir_YEAR_RE_v6, showWarnings = FALSE, recursive = TRUE)

cat("\n", rep("=", 80), "\n", sep = "")
cat("MASTER VISUALIZATION PIPELINE V6 - YEAR RANDOM EFFECT MODELS (INTEGRATED)\n")
cat("V6 Improvements: ARCH interactions, HABMERGED display, YEAR SD annotation\n")
cat(rep("=", 80), "\n", sep = "")
cat("Output directory:", output_dir_YEAR_RE_v6, "\n")
...
detect_model_type_year_re_v6 <- function(model, model_name) {
...
generate_forest_plot_year_re_v6 <- function(model, model_name, model_info) {
    cat("    -> Gerando Forest Plot V6 (com HABMERGED, YEAR SD, interações)...\n")

    # Para DIRICHLET, usar função JSDM existente (importar do original se necessário)
    if (model_info$type == "DIRICHLET") {
        cat("    → Modelo DIRICHLET detectado: usando função JSDM\n")
        # Importar função do script original ou implementar localmente
    ...
    # ─────────────────────────────────────────────────────────────
# V6: Adicionar YEAR SD annotation quando aplicável
# ─────────────────────────────────────────────────────────────
    ...
    # V6: Adicionar nota sobre interações ARCH se presentes
...
### ============================================================================
### V6 IMPROVEMENT 2: MARGINAL EFFECTS COM DETECÇÃO DE INTERAÇÕES ARCH
### ============================================================================

generate_marginal_effects_year_re_v6 <- function(model, model_name, model_info,
                                                resolution = 200, ndraws = 500) {
    cat(sprintf("    -> Gerando PDPs V6 (res=%d, ndraws=%d)...\n", resolution, ndraws))
    ...
    # ─────────────────────────────────────────────────────────────
# V6 CRITICAL: Detectar interações ARCH na fórmula
# ─────────────────────────────────────────────────────────────
    ...
    for (pred in continuous_preds) {
        ...
        # ─────────────────────────────────────────────────────────────
# V6: Verificar se há interação com ARCH para este preditor
# ─────────────────────────────────────────────────────────────
        ...
        if (has_interaction) {
            ...
            newdata_inner$ARCH <- model_data$ARCH[1]  # Usar primeiro nível como referência
            newdata_outer$ARCH <- model_data$ARCH[1]
            # Tentar configurar corretamente os níveis
            ...
            if (!is.null(pred_inner) && !is.null(pred_outer)) {
                ...
                # Ensure ARC names match the color scale
                ...
                p <- ggplot(plot_data, aes(x = x, y = estimate, color = ARC, fill = ARC, group = ARC)) +
                    ...
                    labs(
                        title = paste("Effect of", clean_predictor_name(pred), "× ARC (V6)"),
    ...
    combined <- wrap_plots(plot_list, ncol = 2) +
        plot_annotation(
            title = "Partial Dependence Plots (V6: with ARCH interaction detection)",
...
generate_year_re_diagnostics_v6 <- function(model, model_name) {
    ...
    # ─────────────────────────────────────────────────────────────
# V6: Adicionar teste de significância
# ─────────────────────────────────────────────────────────────
    if (any(!is.na(year_df$SE))) {
        # Significância aproximada: |Estimate| > 2*SE
...
generate_interaction_spotlight_ARCH <- function(model, interaction_term, species, predictor) {
    ...
    if (!is_significant) {
        ...
        # Continuar mesmo assim, mas marcar como não significativo
    }
    ...
    if (length(arch_levels) == 2) {
        ...
        # Usar aproximação baseada em IC para significância da diferença
...
# ─────────────────────────────────────────────────────────────
    if (length(arch_levels) == 2 && "diff_estimate" %in% names(plot_data)) {
        ...
        combined <- (p1 / p2) + plot_annotation(
            title = sprintf("Interaction Spotlight V6: %s × ARCH", predictor),
...
### ============================================================================
### V6: JSDM-SPECIFIC FUNCTIONS (IMPORTED FROM ORIGINAL)
### ============================================================================
...
### ============================================================================
### V6 EXPANDED: JSDM MULTI-PREDICTOR VISUALIZATIONS
### ============================================================================
...
generate_ppc_plots_year_re_v6 <- function(model, model_name, model_info) {
    cat("    -> PPC...\n")
...
generate_diagnostics_year_re_v6 <- function(model, model_name) {
    cat("    -> Diagnósticos...\n")
    ...
    # V6: Adicionar YEAR Rhat se disponível
...
### ============================================================================
### V6: PIPELINE PRINCIPAL
### ============================================================================

process_model_year_re_v6 <- function(rds_file, output_base_dir) {
    ...
    model_info <- detect_model_type_year_re_v6(model, model_name)
    ...
    # ─────────────────────────────────────────────────────────────
# FIGURA 1: Forest Plot V6
# ─────────────────────────────────────────────────────────────
    cat("\n  [1/7] Forest Plot V6 (com HABMERGED, YEAR SD)...\n")
    p_forest <- generate_forest_plot_year_re_v6(model, model_name, model_info)
    if (!is.null(p_forest)) {
...
# ─────────────────────────────────────────────────────────────
# FIGURA 2: Marginal Effects V6 (COM INTERAÇÕES ARCH!)
# ─────────────────────────────────────────────────────────────
    cat("\n  [2/7] Marginal Effects V6 (com ARCH interaction detection)...\n")
    if (model_info$type %in% c("GAUSSIAN", "ZOIB")) {
        p_effects <- generate_marginal_effects_year_re_v6(model, model_name, model_info)
        if (!is.null(p_effects)) {
            ...
            cat("    ✓ FIGURE_2 salvo (com ARCH detection)\n")
        }
    }
...
# ─────────────────────────────────────────────────────────────
    cat("\n  [3/7] PPC...\n")
    p_ppc <- generate_ppc_plots_year_re_v6(model, model_name, model_info)
    if (!is.null(p_ppc)) {
...
# ─────────────────────────────────────────────────────────────
    cat("\n  [4/7] Diagnostics...\n")
    p_diag <- generate_diagnostics_year_re_v6(model, model_name)
    if (!is.null(p_diag)) {
...
# ─────────────────────────────────────────────────────────────
    if (model_info$has_year_re) {
        cat("\n  [EXTRA] YEAR RE Diagnostics V6...\n")
        p_year <- generate_year_re_diagnostics_v6(model, model_name)
        if (!is.null(p_year)) {
...
# ─────────────────────────────────────────────────────────────
# V6 EXTRA: Interaction Spotlight para interações significativas
# ─────────────────────────────────────────────────────────────
    if (model_info$uses_arch) {
...
### ============================================================================
### V6: EXECUÇÃO PRINCIPAL
### ============================================================================

cat("\n", rep("=", 80), "\n", sep = "")
cat("🚀 INICIANDO PIPELINE V6 - YEAR RANDOM EFFECT MODELS (INTEGRATED)\n")
cat("V6 Improvements:\n")
...
winner_files <- discover_winner_models(search_directories_YEAR_RE_v6)

processed_info <- list()
for (rds_file in winner_files) {
    result <- process_model_year_re_v6(rds_file, output_dir_YEAR_RE_v6)
    if (!is.null(result)) processed_info[[length(processed_info) + 1]] <- result
}

cat("\n", rep("=", 80), "\n", sep = "")
cat("📋 RELATÓRIO FINAL - PIPELINE V6 INTEGRADO\n")
cat(rep("=", 80), "\n", sep = "")

cat(sprintf("\n✓ Modelos processados: %d\n", length(processed_info)))
cat(sprintf("📁 Saída: %s\n", output_dir_YEAR_RE_v6))

cat("\nFiguras geradas (V6):\n")
cat("  [ZOIB/GAUSSIAN MODELS:]\n")
cat("  - FIGURE_1_Forest_Plot.png/pdf (com YEAR SD annotation)\n")
cat("  - FIGURE_2_Marginal_Effects.png/pdf (V6: com ARCH interaction detection!)\n")
