...
# ============================================================================
# Full-grid prior sensitivity for JSDM Dirichlet YEAR_RE models
...
base_output_dir <- output_dirs_year_re_prior_sens$JSDM_YEAR_RE
...
for (cv_label in cv_scenarios) {
  ...
  prepared_data <- tryCatch({
    prepare_jsdm_data_with_year(dataset_paths_abundance[[cv_label]], cv_label)
  }, error = function(e) {
  ...
  for (scenario_name in prior_scenarios) {
    ...
    convergence_map <- list()
      ...
      for (model_config in model_combinations_year_re) {
        ...
        cat(sprintf("Response: Dirichlet | Model: %s | CV: %s | Prior: %s\n", 
      ...
      priors_to_use <- build_prior_set_jsdm(
    ...
    if (length(loo_list) > 1) {
      ...
      if (!is.null(cv_comp)) {
        winner_path <- file.path(cv_prior_output_dir, sprintf("WINNER_JSDM_%s_%s.rds", cv_label, scenario_name))
        src_path <- file.path(
          cv_prior_output_dir,
          sprintf("jsdm_year_re_%s_%s_prior_%s.rds", tolower(cv_comp$winner), cv_label, scenario_tag)
        )
...
for (scenario_name in prior_scenarios) {
  ...
  for (reg_key in scenario_keys) {
    ...
    comparison_path <- file.path(
      base_output_dir,
      sprintf("CV_WINDOW_LOO_JSDM_%s_%s_PS_FULLGRID.csv", tolower(model_name), scenario_tag_upper)
    )
    cv_comp <- compare_cv_windows_formal_loo(
      loo_by_cv = cv_window_loo_registry[[reg_key]],
      context_label = sprintf("JSDM_%s_%s", model_name, scenario_name),
  ...
  if (nrow(cv_window_summary) > 0) {
    cv_summary_path <- file.path(
      base_output_dir,
      sprintf("CV_WINDOW_LOO_SUMMARY_JSDM_%s_PS_FULLGRID.csv", scenario_tag_upper)
    )
...
if (nrow(all_results) > 0) {
  results_path <- file.path(base_output_dir, "JSDM_YEAR_RE_combined_results_all_priors_PS_FULLGRID.csv")
  write.csv(all_results, results_path, row.names = FALSE)

  prior_sensitivity <- compute_prior_sensitivity_summary(all_results, model_family_label = "JSDM_YEAR_RE")
  if (nrow(prior_sensitivity) > 0) {
    prior_sensitivity_path <- file.path(base_output_dir, "PRIOR_SENSITIVITY_JSDM_PS_FULLGRID.csv")
    write.csv(prior_sensitivity, prior_sensitivity_path, row.names = FALSE)
  }
}

cat(sprintf("Output directory: %s\\n", base_output_dir))
cat("Script 03 completed (JSDM YEAR_RE prior sensitivity full-grid).\\n")
