...
for (key_name in names(cv_window_loo_registry)) {
  comparison_path <- file.path(
    base_output_dir,
    sprintf("CV_WINDOW_LOO_RGR_%s.csv", tolower(gsub("__", "_", key_name)))
  )
  cv_comp <- compare_cv_windows_formal_loo(
    loo_by_cv = cv_window_loo_registry[[key_name]],
    context_label = sprintf("RGR_%s", key_name),
...
if (nrow(cv_window_summary) > 0) {
  cv_summary_path <- file.path(base_output_dir, "CV_WINDOW_LOO_SUMMARY_RGR.csv")
